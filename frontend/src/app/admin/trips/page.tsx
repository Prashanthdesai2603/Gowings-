  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Link href="/admin/trips/new" className="bg-primary text-white px-4 py-2 rounded-lg font-semibold flex items-center gap-2 hover:bg-primary/90 transition">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          ...
          <div className="flex gap-2">
            <select className="border rounded-lg px-4 py-2 bg-white text-slate-700 outline-none">
              ...
              <tr key={pkg.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition">
                <td className="py-4 px-6">
                  <div className="flex items-center gap-4">
                    <img src={pkg.images?.[0] || pkg.imageUrl || "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?auto=format&fit=crop&w=800&q=80"} alt={pkg.title} className="w-12 h-12 rounded object-cover bg-slate-200" />
                    <div>
                      <p className="font-bold text-slate-800">{pkg.title}</p>
                      <p className="text-xs text-slate-500">{pkg.duration || "Multi-day"} • {pkg.destination?.name || "Global"}</p>
                    </div>
                  </div>
                </td>
                <td className="py-4 px-6 text-slate-600 font-medium">{pkg.category?.name || "Tour"}</td>
                <td className="py-4 px-6 text-slate-800 font-bold">₹{pkg.price.toLocaleString('en-IN')}</td>
                <td className="py-4 px-6">
                  {pkg.isFeatured ? (
                    <span className="bg-amber-100 text-amber-700 px-2 py-1 rounded text-xs font-bold">Featured</span>
                  ) : (
                    <span className="bg-slate-100 text-slate-700 px-2 py-1 rounded text-xs font-bold">Standard</span>
                  )}
                </td>
                <td className="py-4 px-6 text-right">
                  <div className="flex justify-end gap-2">
                    <Link href={`/admin/trips/${pkg.id}/edit`} className="p-2 bg-slate-100 text-slate-600 rounded hover:bg-primary/10 hover:text-primary transition" title="Edit">
                      <Edit size={16} />
                    </Link>
